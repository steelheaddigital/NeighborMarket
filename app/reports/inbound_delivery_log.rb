class InboundDeliveryLog < Prawn::Document
  ...
  def to_pdf(inventory_items)
    ...
    inventory_items.sort_by{|item| [item.inventory_item.user.last_name, item.inventory_item.user.first_name, item.order.user.last_name, item.order.user.first_name]}.map do |item|
    items +=  [[
