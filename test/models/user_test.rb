...
class UserTest < ActiveSupport::TestCase
  ...
  test 'should create user_in_person_setting after create if user is seller' do
    @seller.save

    assert_not_nil @seller.user_in_person_setting
  end

  test 'should not create user_in_person_setting after create if user is not seller' do
    @buyer.save
  ...
  test 'online_payment_processor_configured? returns true if user has paypal express configured' do
    user = users(:approved_seller_user)

    assert user.online_payment_processor_configured?
  end

  test 'online_payment_processor_configured? returns false if user does not paypal express configured' do
    user = users(:unapproved_seller_user)
  ...
  test 'should create user_in_person_setting on create' do
    PaymentProcessorSetting.stub :current_processor_type, 'Online' do
      assert_difference 'UserInPersonSetting.count' do
        @seller.save
        assert !@seller.user_in_person_setting.nil?
        assert !@seller.user_in_person_setting.accept_in_person_payments
      end
    end
  end

  test 'should create user_in_person_setting on create and set accept_in_person_payments to true if InPerson payment processor' do
