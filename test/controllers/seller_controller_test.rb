...
class SellerControllerTest < ActionController::TestCase
  ...
  test "should export csv if commit equals Export to CSV" do
    ...
    assert_match("Item Name,Price Per Unit,Total Units Sold,Total Price", @response.body.to_s)
    assert_match("Carrot,10.0,10,100.0", @response.body.to_s)
    assert_match("test,10.0,5,50.0", @response.body.to_s)
    assert_match("Jam,10.0,10,100.0", @response.body.to_s)
    assert_match("test,10.0,3,30.0", @response.body.to_s)
    assert_response(:success)
  end
